...
void mytest(void) {
	printf("-----STARTING RYAN'S TEST-----\n");
	char* str = "ryanispro";
	char* utstr = utstrdup(str);
	// change capacity to one more
	*(utstr-8) = *(utstr-8) + 1;
	int length_test = utstrlen(utstr);
	if (length_test == 9) {
		printf("utstrlen: PASS\n");
	}
	else {
		printf("length function: FAIL\n");
	}
	char* suffix = "not";
	utstrcat(utstr, suffix);
	printf("%s\n", utstr);
	// change capacity to 
	*(utstr-8) = 5;
	char* str2 = "doesntfit";
